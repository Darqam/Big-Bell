...
module.exports = {
    ...
    async execute(channel) {
		...
		if(!channel_gym) {
			return console.log(`Could not match a gym pattern for ${channel.name}`);
		}

		// From here on, we *should* only have the gym name
		let gym = await channel.client.Gyms.findOne({
			where: {
		...
		if(!gym) {
			const func_return = await chanList.getGymNames(channel.client, channel_gym);
		...
		if(found) {
            ...
            stats.writeStats(channel.client, channel_gym, channel.guild.id, pokemon);
